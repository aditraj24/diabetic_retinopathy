...
export default async function HistoryDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      <div className="mb-6">
        <Link href="/dashboard/history" className="text-sm font-medium text-blue-600 hover:text-blue-800 transition-colors flex items-center gap-1">
          &larr; Back to History
        </Link>
      </div>

      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 bg-white p-5 rounded-2xl border border-gray-200 shadow-sm">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Analysis Review</h1>
          <p className="text-sm text-gray-500 mt-1">Analysed on {dateStr}</p>
        </div>
      </div>

      <div className="flex flex-col lg:flex-row gap-8">
        <div className="w-full lg:w-5/12 xl:w-1/2">
          <div className="bg-white rounded-2xl border border-gray-200 p-3 shadow-sm h-full max-h-[600px] sticky top-24">
            <div className="relative w-full h-full min-h-[400px] rounded-xl overflow-hidden bg-black">
              <Image 
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 sm:p-8">
            <div className="text-center mb-8">
              ...
              <p className="mt-4 text-gray-600 font-medium">
                Predicted with <span className="font-bold text-gray-900">{(analysis.confidence * 100).toFixed(1)}%</span> confidence
              </p>
            </div>
