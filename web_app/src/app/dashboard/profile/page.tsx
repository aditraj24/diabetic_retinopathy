  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        {/* Profile Card */}
        <div className="bg-white border border-gray-200 rounded-md p-6 col-span-1 flex flex-col items-center">
          <div className="w-24 h-24 rounded-full bg-gray-800 text-white flex items-center justify-center text-3xl font-medium mb-4">
            {initial}
          </div>
          <ProfileClient userId={user._id.toString()} username={user.username} initialName={user.displayName} />
          <p className="text-xs text-gray-500 mt-5">Member since {memberSince}</p>
        </div>
        ...
        <div className="col-span-1 lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white border border-gray-200 rounded-md p-5">
              <p className="text-xs font-medium text-gray-500 mb-1">Total Analyses</p>
              <p className="text-2xl font-semibold text-gray-900">{totalAnalyses}</p>
            </div>
            <div className="bg-white border border-gray-200 rounded-md p-5">
              <p className="text-xs font-medium text-gray-500 mb-1">Most Common</p>
              <p className="text-base font-semibold text-gray-900">
                {mostCommonGrade !== -1 ? GRADE_MAP[mostCommonGrade].shortLabel : "N/A"}
              </p>
            </div>
            <div className="bg-white border border-gray-200 rounded-md p-5">
              <p className="text-xs font-medium text-gray-500 mb-1">Last Analysis</p>
              <p className="text-base font-semibold text-gray-900">
              ...
              {[0, 1, 2, 3, 4].map(grade => {
                ...
                return (
                  <div key={grade} className="flex items-center gap-3">
                    <div className="w-20 text-xs font-medium text-gray-600">{info.shortLabel}</div>
                    ...
                    <div className="w-10 text-right text-xs font-medium text-gray-700">{count}</div>
                  </div>
                );
              })}
            </div>
